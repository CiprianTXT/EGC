...
void Lab2::Init()
{
    ...
    meshes["tetrahedron"]->InitFromData(verticesTetrahedron, indicesTetrahedron);
    ...
    // BONUS:
    vector<VertexFormat> verticesDisk
    {
        VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 0.5, 1), glm::vec3(0.2, 0.8, 0.6)),
        VertexFormat(glm::vec3(1, 0,  0), glm::vec3(0, 0, 1)),
        VertexFormat(glm::vec3(0.75, 0.75,  0), glm::vec3(0, 0.5, 1)),
        VertexFormat(glm::vec3(0, 1,  0), glm::vec3(0, 1, 1)),
        VertexFormat(glm::vec3(-0.75, 0.75,  0), glm::vec3(0, 0.5, 1)),
        VertexFormat(glm::vec3(-1, 0,  0), glm::vec3(0, 0, 1)),
        VertexFormat(glm::vec3(-0.75, -0.75,  0), glm::vec3(0, 0.5, 1)),
        VertexFormat(glm::vec3(0, -1,  0), glm::vec3(0, 1, 1)),
        VertexFormat(glm::vec3(0.75, -0.75,  0), glm::vec3(0, 1, 1))
    };

    vector<unsigned int> indicesDisk =
    {
        0, 1, 2, 3, 4, 5, 6, 7, 8, 1
    };

    meshes["disk"] = new Mesh("generated disk");
    meshes["disk"]->InitFromData(verticesDisk, indicesDisk);
    meshes["disk"]->SetDrawMode(GL_TRIANGLE_FAN);
}
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["tetrahedron"], shaders["VertexColor"], glm::vec3(0, 0.5f, 0), glm::vec3(0.25f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(0, 0.5f, 2), glm::vec3(0.25f));

    // BONUS:
    RenderMesh(meshes["disk"], shaders["VertexColor"], glm::vec3(0, 0.5f, 3), glm::vec3(0.25f));
