...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    glUniform1d(loc_elapsed_time, Engine::GetElapsedTime());

    int loc_is_earth = glGetUniformLocation(shader->program, "isEarth");
    glUniform1i(loc_is_earth, isEarth);

    int loc_is_grass = glGetUniformLocation(shader->program, "isGrass");
    glUniform1i(loc_is_grass, isGrass);
