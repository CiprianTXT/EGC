...
{
    {
        ...
        RenderSimpleMesh(meshes["sphere"], shaders["LabShader"], modelMatrix, glm::vec3(0.4, 0.5, 0.8));
...

    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.4, 0.5, 0.1));
    ...
    // Render ground
    {
        ...
        // TODO(student): Add or change the object colors
        RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0, 0.3, 0));
...
void Lab8::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_LEFT))
    {
        ...
        // Control light position using on W, A, S, D, E, Q
        if (window->KeyHold(GLFW_KEY_W)) lightPosition2 -= forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_A)) lightPosition2 -= right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_S)) lightPosition2 += forward * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_D)) lightPosition2 += right * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_E)) lightPosition2 += up * deltaTime * speed;
        if (window->KeyHold(GLFW_KEY_Q)) lightPosition2 -= up * deltaTime * speed;
    }

    if (window->KeyHold(GLFW_KEY_I)) lightDirection = glm::vec3(0, 1, 0);
    if (window->KeyHold(GLFW_KEY_J)) lightDirection = glm::vec3(1, 0, 0);
    if (window->KeyHold(GLFW_KEY_K)) lightDirection = glm::vec3(0, -1, 0);
    if (window->KeyHold(GLFW_KEY_L)) lightDirection = glm::vec3(0, 0, 1);
    if (window->KeyHold(GLFW_KEY_Z)) cut_off += deltaTime * 2;
    if (window->KeyHold(GLFW_KEY_X)) cut_off -= deltaTime * 2;
}
